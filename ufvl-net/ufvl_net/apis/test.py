...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            print(np.min(np.array(time_list)))

            transl_err_list.append(result['trans_error_med'])
            rot_err_list.append(result['rot_err_med'])
            res_ = np.array([transl_err_list, rot_err_list]).T
            print('Median pose error: {}m, {}\u00b0'.format(np.median(res_[:, 0]),
                                                            np.median(res_[:, 1])))
            print('Average pose error: {}m, {}\u00b0'.format(np.mean(res_[:, 0]),
                                                             np.mean(res_[:, 1])))
            print('stddev: {}m, {}\u00b0'.format(np.std(res_[:, 0], ddof=1),
                                                 np.std(res_[:, 1], ddof=1)))
            print("ACC:{}", np.sum((res_[:, 0] <= 0.050) * (res_[:, 1] <= 5)) * 1. / len(res_) * 100)
